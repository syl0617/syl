...
def save_figure(button, event):
    ...
    if file_path:  # If a file path is selected
        plt.savefig(file_path, dpi=dpi, transparent=True)
...
def line_in_sorted_lines(line_to_check, sorted_lines):
    xdata_to_check, ydata_to_check = line_to_check.get_data()

    for line in sorted_lines:
        xdata, ydata = line.get_data()

        if np.array_equal(xdata, xdata_to_check) and np.array_equal(ydata, ydata_to_check):
            return True
...
def animate_convex_hull(vertices, hull_front, hull_top, hull_side, show_labels=True): 
    ...
    for i in range(max(len_front, len_top, len_side)):
        ...
        # Draw lines to points until finding the convex hull
        front_lines = [] 
        top_lines = []
        side_lines = []
        front_sorted_lines = []
        ...
        def animate_line(i):
            ...
            if i < max(len_front, len_top, len_side):
                for j in range(max(len_front, len_top, len_side)):
                    if i < len_front and j < len_front:
                        front_line, = ax_2d_front.plot([vertices[sorted_vertices_front[i], 0], vertices[sorted_vertices_front[j], 0]], 
                            [vertices[sorted_vertices_front[i], 2], vertices[sorted_vertices_front[j], 2]], 'k-')
                        front_lines.append(front_line)
                    if i < len_top and j < len_top:
                        top_line, = ax_2d_top.plot([vertices[sorted_vertices_top[i], 0], vertices[sorted_vertices_top[j], 0]], 
                            [vertices[sorted_vertices_top[i], 1], vertices[sorted_vertices_top[j], 1]], 'k-')
                        top_lines.append(top_line)
                    if i < len_side and j < len_side:
                        side_line, = ax_2d_side.plot([vertices[sorted_vertices_side[i], 1], vertices[sorted_vertices_side[j], 1]], 
                            [vertices[sorted_vertices_side[i], 2], vertices[sorted_vertices_side[j], 2]], 'k-')
                        side_lines.append(side_line)
                    plt.pause(0.1)
                front_sorted_line, = ax_2d_front.plot(vertices[sorted_vertices_front[i:i+2], 0], vertices[sorted_vertices_front[i:i+2], 2], 'k-')
                front_sorted_line.set_visible(False)
                front_sorted_lines.append(front_sorted_line)
                top_sorted_line, = ax_2d_top.plot(vertices[sorted_vertices_top[i:i+2], 0], vertices[sorted_vertices_top[i:i+2], 1], 'k-')
                top_sorted_line.set_visible(False)
                top_sorted_lines.append(top_sorted_line)
                side_sorted_line, = ax_2d_side.plot(vertices[sorted_vertices_side[i:i+2], 1], vertices[sorted_vertices_side[i:i+2], 2], 'k-')
                side_sorted_line.set_visible(False)
                side_sorted_lines.append(side_sorted_line)
                plt.pause(0.05)
            ...
            for line in front_lines:
                if i <= len_front - 1:
                    if line in ax_2d_front.lines:
                        if line_in_sorted_lines(line, front_sorted_lines):
                            pass
                        else:
                            line.remove()             
            ...
            for line in top_lines:
                if i <= len_top - 1:
                    if line in ax_2d_top.lines:
                        if line_in_sorted_lines(line, top_sorted_lines):
                            pass
                        else:
                            line.remove()             
            ...
            for line in side_lines:
                if i <= len_side - 1:
                    if line in ax_2d_side.lines:
                        if line_in_sorted_lines(line, side_sorted_lines):
                            pass
                        else:
                            line.remove()             
        ...
        ani = animation.FuncAnimation(fig, animate_line, frames=150, interval=150)
